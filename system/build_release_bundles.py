...
def build(root: Path, output: Path) -> dict[str, object]:
    ...
    copy_files(paper, artifact / "paper_revised", ARXIV_SOURCE_FILES)
    shutil.copy2(
        paper / "README_SUBMISSION.txt",
        artifact / "paper_revised" / "README_SUBMISSION.txt",
    )
    copy_files(
        root / "submission",
        artifact / "submission",
